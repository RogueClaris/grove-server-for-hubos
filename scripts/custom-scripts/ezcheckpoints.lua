...
local help_button = require('scripts/custom-scripts/help_button')
...
Net:on("object_interaction", function(event)
    ...
    local custom_message = checkpoint_object.custom_properties["Unlock Message"]

    if not required_item then return end

    local lowercase_item = string.lower(required_item)
    local item_count = tonumber(0)

    if lowercase_item == "money" then
        item_count = Net.get_player_money(player_id)
    else
        item_count = player_data.count_player_item(player_id, required_item)
    end
    local necessary_count = tonumber(1)
    local is_hide_forever = false

    if checkpoint_object.custom_properties["Once"] ~= nil then
        is_hide_forever = tostring(checkpoint_object.custom_properties["Once"]) == "true"
    end
    ...
    if item_count >= necessary_count or lowercase_item == "flag" and help_button.flags_map[required_item] == true then
        ...
        if dialogue_check then
            if custom_message ~= nil then
                Net.message_player(player_id, custom_message)
            else
                Net.message_player(player_id,
                    "You unlocked " .. checkpoint_object.name .. " with " .. required_item .. "!")
            end
        end
        ...
        Async.sleep(tonumber(checkpoint_object.custom_properties["Disappear Cooldown"])).and_then(function()
            if is_hide_forever == true then
                --Hide permanently if needed
                player_data.hide_target_from_player(player_id, area_id, object_id, "OBJECT", true)
            else
                --Otherwise hide it until disconnect
                player_data.hide_target_from_player(player_id, area_id, object_id, "OBJECT", false)
            end
            local relock_data = {
                type = "tile",
                gid = lockedGID,
            ...
            if take_item then
                if lowercase_item == "money" then
                    player_data.update_player_money(player_id, -necessary_count)
                else
                    player_data.update_player_item(player_id, required_item, -necessary_count)
                end
            end
